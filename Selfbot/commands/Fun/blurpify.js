...
module.exports = {
    ...
    */
    run: async (client, message, args) => {
        ...
        const response = await fetch(`https://nekobot.xyz/api/imagegen?type=blurpify&image=${encodeURIComponent(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png?size=4096`)}`).then(r => r.json()).catch(() => null);
        if (response && response.message)
            message.edit(client.language(
                `*[${user.username} est blurpifié](${response.message}) *`,
                `*[${user.username} is blurpified](${response.message}) *`
            ));
